  ...
  const filterProducts = async () => {
    try {
      const response = await axios.post(
        "http://localhost:8000/api/filter-products",
        filters
      );
    ...
    const fetchProductsForCategory = async () => {
      try {
        const response = await fetch(
          "http://localhost:8000/api/product/category/" + selectedCategory[0]
        );
        const data = await response.json();
    ...
    <div>
      ...
      <div className="filter container">
        <h1 className="search-heading container">
          Bütün məhsullar ({products.length})
        </h1>
      ...
      <div
        className={`productAndFilter container ${
          isFilterModalOpen ? "filter-open" : ""
        }`}
      >
